...
class MetaDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if ds_labels is None:
            self.ds_labels = ["dummy"] * len(self.datasets)
        else:
            self.ds_labels = ds_labels
        assert len(self.ds_labels) == len(self.datasets), "ds_labels must match number of datasets"
    ...
    def __getitem__(self, idx):
        """
        Fetches a sample from the appropriate sub-dataset. The dataset label and id for the sample
        can be retrieved using get_ds_label(idx) and get_ds_id(idx) if needed for downstream tasks.
        """
        ...
        meta["index"] = idx
        # If you want to return the dataset label or id, you can add them to meta here:
        # meta["ds_label"] = self.ds_labels[ds_ind]
        # meta["ds_id"] = self.ds_labels_to_ids[self.ds_labels[ds_ind]]
        return meta
